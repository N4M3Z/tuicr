...
Keyboard arrow keys / `h` / `l` are wired to `Action::ScrollLeft` and `Action::ScrollRight`, which carry two responsibilities: scroll the diff viewport horizontally, and shift focus between panes at the edges (see [FEAT-0012][FEAT-0012], [FEAT-0014][FEAT-0014]). Mouse and trackpad horizontal events (`MouseEventKind::ScrollLeft` / `ScrollRight`) initially routed through the same `Action` path.

That principle broke the moment a trackpad user gestured horizontally. A two-finger swipe over the diff at `scroll_x = 0` shifted focus to the file list. A swipe over the file list dismissed the panel. A swipe during an active `VisualSelect` aborted the selection. None of these are gestures the user meant to perform: the trackpad is a scroll input, not a panel-management input.
...
[FEAT-0014]: FEAT-0014 Sliding Panes.md
